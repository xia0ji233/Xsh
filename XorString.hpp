...
/* GCC/Clang 兼容：__forceinline 是 MSVC 专用 */
#ifndef __forceinline
#define __forceinline __attribute__((always_inline)) inline
#endif

/* GCC/Clang 需要 immintrin.h 提供 __m128i/__m256i */
#if !defined(_M_ARM64) && !defined(__aarch64__) && !defined(_M_ARM) && !defined(__arm__)
#if defined(__SSE2__) || defined(_M_X64) || defined(__amd64__) || defined(_M_IX86) || defined(__i386__)
#include <immintrin.h>
#endif
#endif
...
static __forceinline __m128i my_mm_load_si128(const void *p)
{
...
static __forceinline void my_mm_store_si128(void *p, __m128i v)
{
    __asm__ volatile("movdqa (%1), %%xmm0\n\t"
                     "movdqa %%xmm0, (%0)\n\t"
                     :
...
static __forceinline __m128i my_mm_xor_si128(__m128i a, __m128i b)
{
    alignas(16) __m128i out;
    __asm__ volatile("movdqa (%1), %%xmm0\n\t"
                     "pxor   (%2), %%xmm0\n\t"
...
static __forceinline __m256i my_mm256_xor_si256(__m256i a, __m256i b)
{
    alignas(32) __m256i out;
    __asm__ volatile("vmovdqa (%1), %%ymm0\n\t"
                     "vpxor   (%2), %%ymm0, %%ymm0\n\t"
...
#define XorWString(str) xorstr(str).crypt_get()
//#define XorString(str) str
//#define XorWString(str) str
#ifdef _MSC_VER
...
class xor_string<CharT, Size, std::integer_sequence<std::uint64_t, Keys...>, std::index_sequence<Indices...>>
{
    ...
    constexpr static inline std::uint64_t alignment = 32;  // 256 bit optimize must align to 32
#else
  ...
  public:
    ...
    XORSTR_FORCEINLINE void crypt() noexcept
    {
        ...
        ((Indices >= sizeof(_storage) / 16
              ? static_cast<void>(0)
              : vst1q_u64(reinterpret_cast<uint64_t *>(_storage) + Indices * 2,
        ...
        ((Indices >= sizeof(_storage) / 32
              ? static_cast<void>(0)
              : my_mm256_store_si256(
                    reinterpret_cast<__m256i *>(_storage) + Indices,
                    my_mm256_xor_si256(my_mm256_load_si256(reinterpret_cast<const __m256i *>(_storage) + Indices),
                                     my_mm256_load_si256(reinterpret_cast<const __m256i *>(keys) + Indices)))),
         ...);

        if constexpr (sizeof(_storage) % 32 != 0)
            my_mm_store_si128(
                reinterpret_cast<__m128i *>(_storage + sizeof...(Keys) - 2),
                my_mm_xor_si128(my_mm_load_si128(reinterpret_cast<const __m128i *>(_storage + sizeof...(Keys) - 2)),
                              my_mm_load_si128(reinterpret_cast<const __m128i *>(keys + sizeof...(Keys) - 2))));
#else
        ((Indices >= sizeof(_storage) / 16
              ? static_cast<void>(0)
              : my_mm_store_si128(reinterpret_cast<__m128i *>(_storage) + Indices,
                                my_mm_xor_si128(my_mm_load_si128(reinterpret_cast<const __m128i *>(_storage) + Indices),
                                              my_mm_load_si128(reinterpret_cast<const __m128i *>(keys) + Indices)))),
         ...);
#endif
    }
