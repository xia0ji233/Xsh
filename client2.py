#!/usr/bin/env python3
"""
client2.py - 通过 HTTP 获取 /.xia0ji233/flag 加密文件并解密
用法:
    python client2.py <ip> [port]                   # 单次获取
    python client2.py <ip> [port] --loop 5          # 每5秒循环获取
    python client2.py targets.txt [port]            # 批量（文件每行一个IP）
"""
import sys
import time
import os
...
def fetch_flag_http(ip: str, port: int) -> str:
    url = f'http://{ip}:{port}{FLAG_PATH}'
